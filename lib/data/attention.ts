...
export type AttentionItem =
  ...
  /** A goal, like a milestone or to-do, gets its own actions (edit due date / change status) rather than Dismiss (KD-028). */
  | (BaseAttentionItem & { kind: "goal"; goalId: string });
...
function toAttentionItem(record: NeedsAttentionEligible): AttentionItem {
  switch (record.kind) {
    ...
    case "goal":
      return { key: dismissalKey("goal", record.id, OVERDUE_NOTIFICATION_TYPE.goal, record.targetDate), kind: "goal", goalId: record.id, title: record.name, context: "Overdue goal", date: record.targetDate.toISOString(), timestamp: record.targetDate.getTime(), href: `/goals/${record.id}` };
  }
}
