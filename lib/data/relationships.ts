...
export async function getRelationshipMap(defaultSelfName?: string): Promise<RelationshipMapData> {
  ...
  return {
    people: people.map((person) => ({ id: person.id, name: person.name, detail: person.isSelf ? "You" : person.category || "Relationship", x: person.positionX, y: person.positionY, size: person.bubbleSize, color: person.color, icon: person.icon as RelationshipMapData["people"][number]["icon"], selfRelationship: { practices: person.selfPractices.map(({ title, cadence }) => ({ title, cadence: cadence || "" })), reflections: person.selfReflections.map(({ text, reflectedAt }) => ({ text, date: reflectedAt.toISOString().slice(0, 10) })), importantDates: person.selfImportantDates.map(({ label, date }) => ({ label, date: date.toISOString().slice(0, 10) })), notes: person.selfNotes || "" } })),
