...
export async function getCalendarItems(start: Date, end: Date): Promise<KinesisCalendarItem[]> {
  ...
  const [goals, documents, importantDates, practices, customItems] = await Promise.all([
    ...
    prisma.document.findMany({ where: { userId: user.id }, select: { id: true, name: true, type: true, expiryDate: true, notifications: { where: { reminderAt: { gte: start, lte: end } }, select: { id: true, reminderAt: true, message: true } } } }),
  ...
  for (const goal of goals) {
    ...
    for (const milestone of goal.milestones) if (milestone.dueDate) add({ id: `milestone-${milestone.id}`, title: `${milestone.name} due`, kind: "DATED", date: milestone.dueDate, sourceType: "MILESTONE", sourceObjectId: goal.id, sourceModule: goal.name, href: `/goals/${goal.id}`, detail: milestone.completed ? "Completed milestone" : "Milestone due date" });
  }
