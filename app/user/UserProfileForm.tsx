...
type UserProfile = { preferredName: string | null };
...
function Field({ label, name, value, hint }: { label: string; name: string; value: string; hint?: string }) {
  return <label className="block text-sm font-medium text-zinc-700">{label}<input name={name} defaultValue={value} className="mt-2 h-12 w-full rounded-xl border border-zinc-200 px-4 outline-none transition focus:border-zinc-400" />{hint && <span className="mt-2 block text-xs font-normal text-zinc-400">{hint}</span>}</label>;
}
