...
export function UserProfileForm({ user }: { user: UserProfile }) {
  ...
  return <form action={formAction} className="mt-8 space-y-5 rounded-3xl border border-zinc-200/80 bg-white p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
    ...
    <Field label="Email" name="email" value={user.email} type="email" required />
...
function Field({ label, name, value, required, type = "text", hint }: { label: string; name: string; value: string; required?: boolean; type?: string; hint?: string }) {
  return <label className="block text-sm font-medium text-zinc-700">{label}<input name={name} type={type} defaultValue={value} required={required} className="mt-2 h-12 w-full rounded-xl border border-zinc-200 px-4 outline-none transition focus:border-zinc-400" />{hint && <span className="mt-2 block text-xs font-normal text-zinc-400">{hint}</span>}</label>;
}
