  ...
  return <form action={formAction} className="mt-6 space-y-4">
    ...
    {!recurring ? <>
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Interest rate"><div className="relative"><input name="rate" type="number" min="0" step="0.01" value={rateInput} onChange={(event) => setRateInput(event.target.value)} placeholder="Optional" className={`${FIELD_CLASS} pr-10`}/><span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-400">%</span></div></Field>
      </div>
