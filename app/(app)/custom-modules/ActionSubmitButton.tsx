...
export function ActionSubmitButton({ idleLabel, pendingLabel, tone = "primary", iconOnly = false }: { idleLabel: string; pendingLabel: string; tone?: "primary" | "danger"; iconOnly?: boolean }) {
  ...
  return <button disabled={pending} className={`inline-flex min-w-32 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold transition disabled:cursor-wait disabled:opacity-70 ${toneClass}`}>{pending ? <LoaderCircle className="h-4 w-4 animate-spin"/> : tone === "primary" && <Check className="h-4 w-4"/>}{pending ? pendingLabel : idleLabel}</button>;
}
