...
export function DeleteModuleButton({ moduleId, moduleName, itemCount }: { moduleId: string; moduleName: string; itemCount: number }) {
  ...
  if (itemCount === 0) return <form action={action}><ActionSubmitButton iconOnly tone="danger" idleLabel={`Delete ${moduleName}`} pendingLabel="Deleting module" /></form>;
  ...
  return <>
    ...
    {confirming && <div role="dialog" aria-modal="true" aria-labelledby="delete-module-title" className={`fixed inset-0 ${Z_INDEX.overlay} flex items-center justify-center bg-zinc-950/40 p-4 backdrop-blur-sm`} onMouseDown={() => setConfirming(false)}>
      <div className="w-full max-w-md rounded-[28px] bg-white p-7 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="mt-7 flex justify-end gap-3"><button type="button" onClick={() => setConfirming(false)} className="rounded-2xl px-5 py-3 text-sm font-semibold text-zinc-500 hover:bg-zinc-100">Cancel</button><form action={action}><ActionSubmitButton tone="danger" idleLabel="Yes, delete module" pendingLabel="Deleting…" /></form></div>
      </div>
    </div>}
  </>;
}
