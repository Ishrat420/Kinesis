...
export default async function CustomModulePage({ params }: { params: Promise<{ moduleId: string }> }) {
  ...
  const hasTemplateDueDate = customModule.templateId ? Boolean(await getTemplateDueDateFieldId(customModule.templateId)) : false;
  ...
  return <ModuleContent width="standard">
    <ModuleHeader title={customModule.name} description={customModule.description} actions={<><DeleteModuleButton moduleId={customModule.id} moduleName={customModule.name} itemCount={customModule.items.length} /><NewItemButton moduleId={customModule.id} linkOptions={linkOptions} hasTemplateDueDate={hasTemplateDueDate} /></>} />
...
function ItemSection({ title, items, moduleId, color, icon }: { title: string; items: Item[]; moduleId: string; color: string; icon: string }) {
  ...
  return <section><div className="mb-3 flex items-center gap-2"><h2 className="text-lg font-semibold">{title}</h2><span className="rounded-full bg-zinc-200/70 px-2.5 py-1 text-xs font-semibold text-zinc-500">{items.length}</span></div><div className="space-y-3">{items.map((item) => <Link key={item.id} href={`/custom-modules/${moduleId}/items/${item.id}`} className="grid grid-cols-[44px_minmax(0,1fr)] items-center gap-x-4 gap-y-2 rounded-2xl border border-zinc-200/80 bg-white p-4 transition hover:-translate-y-0.5 hover:bg-zinc-50 hover:shadow-md sm:grid-cols-[44px_minmax(0,1fr)_auto_24px]"><span className="flex h-11 w-11 items-center justify-center rounded-2xl text-zinc-700" style={{ backgroundColor: `color-mix(in srgb, ${color} 10%, white)` }}><CustomModuleIcon name={icon} className="h-5 w-5"/></span><div className="min-w-0"><h3 className="truncate font-semibold">{item.name}</h3><p className="mt-0.5 truncate text-sm text-zinc-500">{item.notes || (item.fields.length ? `${item.fields.length} custom field${item.fields.length === 1 ? "" : "s"}` : "No details added")}</p></div><span className="col-span-2 w-fit rounded-full bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-500 sm:col-span-1">{item.archived ? "Archived" : "Active"}</span><ChevronRight className="hidden h-5 w-5 text-zinc-300 sm:block"/></Link>)}</div></section>;
}
