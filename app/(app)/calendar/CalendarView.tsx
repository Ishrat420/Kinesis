...
function ItemPill({ item, onClick }: { item: KinesisCalendarItem; onClick: () => void }) { const { locale } = useFormatPreferences(); return <button onClick={onClick} title={item.title} className={`flex w-full items-center gap-1 overflow-hidden rounded-md px-1.5 py-1 text-left text-[11px] font-semibold leading-4 ring-1 ${item.kind === "SCHEDULED" ? "bg-teal-50 text-teal-900 ring-teal-200" : "bg-violet-50 text-violet-900 ring-violet-200"}`}>{item.kind === "SCHEDULED" && <Clock3 className="h-3 w-3 shrink-0"/>}{item.startTime && <span className="shrink-0">{displayTime(item.startTime, locale)}</span>}<span className="truncate">{item.title}</span>{item.recurring && <Repeat2 className="ml-auto h-3 w-3 shrink-0"/>}</button>; }
