...
import { ChevronRight, Cloud, Download, ExternalLink, LayoutTemplate, Plug, UserRound } from "lucide-react";
...
export default async function SettingsPage() {
  ...
  return <ModuleContent width="standard">
    ...
    <Link href="/settings/templates" className="my-6 flex items-center justify-between gap-5 rounded-3xl border border-zinc-200/80 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-center gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-100"><LayoutTemplate className="h-5 w-5" /></span><div><h2 className="text-lg font-semibold">Templates</h2><p className="text-sm text-zinc-500">Reusable field structures a module can start new objects from.</p></div></div><ChevronRight className="h-5 w-5 shrink-0 text-zinc-300" /></Link>
