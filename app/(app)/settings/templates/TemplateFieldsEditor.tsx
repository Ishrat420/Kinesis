...
export function TemplateFieldsEditor({ initialFields, locked }: { initialFields: TemplateFieldInput[]; locked: boolean }) {
  ...
  const payload = useMemo(
    () => JSON.stringify(fields.filter((field) => field.label.trim()).map(({ id, label, type }) => ({ id, label, type }))),
    [fields],
  );
  ...
  return (
    <fieldset>
      ...
      <button
        type="button"
        onClick={addField}
        className={`${fields.length ? "mt-3" : "mt-2"} inline-flex items-center gap-2 rounded-xl border border-dashed border-zinc-300 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-600 transition hover:border-zinc-400 hover:bg-zinc-50 hover:text-zinc-950`}
      >
