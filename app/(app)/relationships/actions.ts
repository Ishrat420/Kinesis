...
export async function saveRelationshipMap(data: RelationshipMapData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const person of data.people) await tx.person.create({ data: { id: person.id, userId: user.id, name: person.name, category: isSelfPerson(person) ? null : person.detail, isSelf: isSelfPerson(person), positionX: person.x, positionY: person.y, bubbleSize: person.size, color: person.color, icon: person.icon, selfNotes: person.selfRelationship.notes || null, selfPractices: { create: person.selfRelationship.practices.map((practice, position) => ({ id: crypto.randomUUID(), ...practice, position })) }, selfReflections: { create: person.selfRelationship.reflections.map((reflection) => ({ id: crypto.randomUUID(), text: reflection.text, reflectedAt: new Date(`${reflection.date}T00:00:00.000Z`) })) }, selfImportantDates: { create: person.selfRelationship.importantDates.map((date) => ({ id: crypto.randomUUID(), label: date.label, date: new Date(`${date.date}T00:00:00.000Z`) })) } } });
