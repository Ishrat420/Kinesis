...
export default async function SettingsPage() {
  ...
  const management = user.role === "OWNER" ? await Promise.all([
    prisma.user.findMany({ orderBy: { createdAt: "asc" }, select: { id: true, firstName: true, lastName: true, preferredName: true, email: true, role: true, status: true } }),
    prisma.userInvitation.findMany({ where: { acceptedAt: null, revokedAt: null }, orderBy: { createdAt: "desc" }, select: { id: true, email: true, createdAt: true } }),
  ]) : null;
  return <main className="min-h-screen bg-[#f7f8fb] text-zinc-950"><Topbar /><div className="flex"><Sidebar /><section className="min-w-0 flex-1 px-6 py-8 md:px-10"><div className="max-w-4xl">
    ...
    {management && <UserManagement members={management[0].map((member) => ({ id: member.id, name: member.preferredName?.trim() || `${member.firstName} ${member.lastName}`.trim(), email: member.email, role: member.role, status: member.status }))} invitations={management[1].map((invitation) => ({ ...invitation, createdAt: invitation.createdAt.toISOString() }))} />}
