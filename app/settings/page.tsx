...
export default async function SettingsPage() {
  ...
  return <main className="min-h-screen bg-[#f7f8fb] text-zinc-950"><Topbar /><div className="flex"><Sidebar /><section className="min-w-0 flex-1 px-6 py-8 md:px-10"><div className="max-w-4xl">
    ...
    <section id="privacy" className="mt-6 rounded-3xl border border-zinc-200/80 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"><h2 className="text-lg font-semibold">Data &amp; privacy</h2><p className="mt-1 text-sm text-zinc-500">Your information belongs to you. Download a copy or start over.</p><div className="mt-5 space-y-4"><div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-zinc-100 p-4"><div className="flex items-center gap-3"><Download className="h-5 w-5" /><div><p className="text-sm font-medium">Export your data</p><p className="text-xs text-zinc-500">Download everything in a portable JSON file.</p></div></div><a href="/api/settings/export" download className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-semibold transition hover:bg-zinc-50">Export JSON</a></div><div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-red-100 p-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-red-50"><Cloud className="h-4 w-4 text-red-600" /></span><div><p className="text-sm font-medium text-red-700">Delete your data</p><p className="text-xs text-zinc-500">Permanently remove everything and return to a clean slate.</p></div></div><DeleteDataButton /></div></div></section>
