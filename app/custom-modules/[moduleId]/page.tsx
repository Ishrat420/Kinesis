...
export default async function CustomModulePage({ params }: { params: Promise<{ moduleId: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f7f8fb] text-zinc-950"><Topbar /><div className="flex"><Sidebar /><section className="min-w-0 flex-1 px-6 py-8 md:px-10"><div className="max-w-5xl">
    <header className="flex flex-wrap items-start justify-between gap-5"><div className="flex items-start gap-4"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-zinc-700 shadow-sm" style={{ backgroundColor: customModule.color }}><CustomModuleIcon name={customModule.icon} className="h-7 w-7" /></span><div><h1 className="text-[38px] font-semibold leading-none tracking-tight">{customModule.name}</h1>{customModule.description && <p className="mt-3 max-w-2xl text-base leading-7 text-zinc-500">{customModule.description}</p>}</div></div><NewItemButton moduleId={customModule.id} color={customModule.color} /></header>
    {!customModule.items.length ? <div className="mt-16 rounded-[28px] border border-dashed border-zinc-300 bg-white px-6 py-16 text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl text-zinc-700" style={{ backgroundColor: customModule.color }}><CustomModuleIcon name={customModule.icon} className="h-7 w-7" /></span><h2 className="mt-5 text-xl font-semibold">Nothing here yet.</h2><p className="mt-2 text-zinc-500">Create your first object to get started.</p></div> : <div className="mt-10 space-y-8">
