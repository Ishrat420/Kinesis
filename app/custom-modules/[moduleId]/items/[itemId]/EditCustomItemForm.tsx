...
export function EditCustomItemForm({ moduleId, item }: { moduleId: string; item: EditableItem }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <div className="flex items-center justify-between rounded-xl border border-zinc-200 px-4 py-3"><div><p className="text-sm font-medium text-zinc-700">Archive status</p><p className="text-sm text-zinc-400">Archived items are hidden from the active list.</p></div><button type="button" aria-pressed={archived} onClick={() => setArchived((current) => !current)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${archived ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}>{archived ? "Archived" : "Not Archived"}</button><input type="hidden" name="archived" value={String(archived)}/></div>
    <div className="flex justify-end border-t border-zinc-100 pt-5"><button className="inline-flex items-center gap-2 rounded-xl bg-zinc-950 px-5 py-2.5 text-sm font-semibold text-white hover:bg-black"><Save className="h-4 w-4"/> Save changes</button></div>
  </form>;
}
