...
export function EditCustomItemForm({ moduleId, item }: { moduleId: string; item: EditableItem }) {
  ...
  return <form action={saveItem} className="space-y-5">
    ...
    <div className="flex items-center justify-between border-t border-zinc-100 pt-5"><span role="status" className={`inline-flex items-center gap-2 text-sm font-medium text-emerald-600 transition ${saveState === "saved" ? "opacity-100" : "opacity-0"}`}><CheckCircle2 className="h-4 w-4"/> Changes saved</span><button disabled={saveState === "saving"} className="inline-flex min-w-36 items-center justify-center gap-2 rounded-xl bg-zinc-950 px-5 py-2.5 text-sm font-semibold text-white hover:bg-black disabled:cursor-wait disabled:opacity-70">{saveState === "saving" ? <LoaderCircle className="h-4 w-4 animate-spin"/> : saveState === "saved" ? <CheckCircle2 className="h-4 w-4"/> : <Save className="h-4 w-4"/>}{saveState === "saving" ? "Saving…" : saveState === "saved" ? "Saved" : "Save changes"}</button></div>
  </form>;
}
