...
export function EditCustomItemForm({ moduleId, item, color }: { moduleId: string; item: EditableItem; color: string }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <div className="flex justify-end border-t border-zinc-100 pt-5"><button className="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-zinc-800" style={{ backgroundColor: `color-mix(in srgb, ${color} 10%, white)` }}><Save className="h-4 w-4"/> Save changes</button></div>
  </form>;
}
