...
export default async function CustomItemPage({ params }: { params: Promise<{ moduleId: string; itemId: string }> }) {
  ...
  const item = await getCustomItem(moduleId, itemId);
  if (!item) notFound();
  return <main className="min-h-screen bg-[#f7f8fb] text-zinc-950"><Topbar/><div className="flex"><Sidebar/><section className="min-w-0 flex-1 px-6 py-8 md:px-10"><div className="max-w-4xl">
    ...
    <header className="flex items-start justify-between gap-5"><div className="flex items-start gap-4"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-zinc-700" style={{ backgroundColor: item.module.color }}><CustomModuleIcon name={item.module.icon} className="h-7 w-7"/></span><div><p className="text-sm font-medium text-zinc-400">{item.module.name}</p><h1 className="mt-1 text-[38px] font-semibold leading-none tracking-tight">{item.name}</h1></div></div><form action={deleteCustomItemAction.bind(null, moduleId, item.id)}><button className="inline-flex items-center gap-2 rounded-xl border border-red-200 bg-white px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4"/> Delete</button></form></header>
    <section className="mt-8 rounded-3xl border border-zinc-200/80 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"><EditCustomItemForm moduleId={moduleId} color={item.module.color} item={{ id: item.id, name: item.name, notes: item.notes ?? "", reminder: item.reminder?.toISOString().slice(0, 10) ?? "", link: item.link ?? "", archived: item.archived, fields: item.fields }} /></section>
