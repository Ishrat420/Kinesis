 ...
 *
 * KD-040 removed the fixed Due Date input entirely -- a due date is now only
 * reachable through a module's template's own Due Date field (KD-038), so
 * every case here goes through one rather than a plain form field.
 */
...
async function makeTemplatedModule() {
  await prisma.template.create({
    data: {
      id: "template-1", userId: owner, name: "Car Renewal",
      fields: { create: [{ id: "field-due", label: "Due date", type: "DATE", position: 0, isDueDate: true }] },
    },
  });
  return prisma.customModule.create({ data: { id: "module-1", userId: owner, name: "Car", normalizedName: "car", icon: "car", color: "#111111", templateId: "template-1" } });
}

const form = (values: Record<string, string>, dueDateValue?: string) => {
  const data = new FormData();
  for (const [key, value] of Object.entries(values)) data.set(key, value);
  if (dueDateValue !== undefined) {
    data.set(TEMPLATE_FIELD_VALUES_FORM_KEY, JSON.stringify([{ templateFieldId: "field-due", value: dueDateValue, targetObjectIds: [] }]));
  }
  return data;
};
...
describe.sequential("a custom item's due date", () => {
  ...
  it("is stored at midnight, not noon", async () => {
    const result = await createCustomItemAction("module-1", {}, form({ name: "Service the car" }, "2027-03-15"));
  ...
  it("rejects a date that only looks valid, the way a To-Do's due date already does", async () => {
    ...
    const result = await createCustomItemAction("module-1", {}, form({ name: "Service the car" }, "2026-02-30"));
  ...
  it("has no due date at all for a module whose template carries no Due Date field", async () => {
    await prisma.customItem.deleteMany({ where: { moduleId: "module-1" } });
    await prisma.customModule.update({ where: { id: "module-1" }, data: { templateId: null } });

    const result = await createCustomItemAction("module-1", {}, form({ name: "Untemplated item" }, "2027-03-15"));

    expect(result.error).toBeUndefined();
    await expect(prisma.customItem.findFirstOrThrow({ where: { name: "Untemplated item" }, select: { dueDate: true } }))
      .resolves.toEqual({ dueDate: null });
  });
});
