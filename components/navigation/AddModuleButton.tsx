    ...
    {open && <div role="dialog" aria-modal="true" aria-labelledby="create-module-title" className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 p-4 backdrop-blur-sm" onMouseDown={() => setOpen(false)}>
      <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-[28px] bg-white p-7 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <form action={formAction} className="mt-7 space-y-5">
          ...
          <fieldset><legend className="text-sm font-semibold">Icon</legend><div className="mt-2 grid grid-cols-7 gap-2">{Object.keys(CUSTOM_MODULE_ICONS).map((name) => <button key={name} type="button" title={name} aria-label={`${name} icon`} aria-pressed={icon === name} onClick={() => setIcon(name as CustomModuleIconName)} className={`flex aspect-square items-center justify-center rounded-xl border transition ${icon === name ? "border-zinc-950 bg-zinc-950 text-white" : "border-zinc-200 text-zinc-500 hover:bg-zinc-50"}`}><CustomModuleIcon name={name} className="h-5 w-5" /></button>)}</div><input type="hidden" name="icon" value={icon} /></fieldset>
          <fieldset><legend className="text-sm font-semibold">Colour</legend><div className="mt-2 flex flex-wrap items-center gap-2">{colors.map((swatch) => <button key={swatch} type="button" aria-label={`Select colour ${swatch}`} aria-pressed={color === swatch} onClick={() => setColor(swatch)} className="flex h-9 w-9 items-center justify-center rounded-full ring-offset-2 transition hover:scale-105" style={{ backgroundColor: swatch, boxShadow: color === swatch ? `0 0 0 2px white, 0 0 0 4px ${swatch}` : undefined }}>{color === swatch && <Check className="h-4 w-4 text-white" />}</button>)}<label className="relative h-9 w-9 overflow-hidden rounded-full border-2 border-dashed border-zinc-300" title="Custom colour"><input type="color" value={color} onChange={(event) => setColor(event.target.value)} className="absolute -inset-2 h-14 w-14 cursor-pointer" /><span className="sr-only">Custom colour</span></label></div><input type="hidden" name="color" value={color} /></fieldset>
