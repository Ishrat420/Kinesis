...
export function AddModuleButton({ templates }: { templates: { id: string; name: string; isStarter: boolean }[] }) {
  ...
  return <>
    ...
    {/* Mounted only while open, not just hidden: this button lives in the
        sidebar and never itself unmounts, so useActionState's success state
...
function CreateModuleModal({ templates, onClose }: { templates: { id: string; name: string; isStarter: boolean }[]; onClose: () => void }) {
