...
import { Bell, CalendarClock, CheckCheck, Clock3, Flag, Heart, TriangleAlert, X } from "lucide-react";
          ...
          <div className="max-h-[calc(100dvh-11rem)] overflow-y-auto p-2 sm:max-h-[480px]">
            {notifications.length === 0 ? (
              ...
              <Link key={notification.id} href={notification.actionUrl} onClick={() => read(notification)} className={`group flex gap-3 rounded-2xl px-3 py-3.5 transition hover:bg-zinc-50 ${readIds.has(notification.id) ? "opacity-70" : "bg-zinc-50/70"}`}>
                <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${notification.type === "EXPIRED" ? "bg-red-50 text-red-600" : isMilestone ? "bg-violet-50 text-violet-700" : isRelationshipDate ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>
                  {notification.type === "EXPIRED" ? <TriangleAlert className="h-5 w-5" /> : isMilestone ? <Flag className="h-5 w-5" /> : isRelationshipDate ? <Heart className="h-5 w-5" /> : <CalendarClock className="h-5 w-5" />}
                </span>
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-400"><span className="flex items-center gap-1"><Clock3 className="h-3 w-3" />{notification.documentType ?? "Document"}</span>{notification.expiryDate && <span>{isMilestone ? "Due" : isRelationshipDate ? "Occurs" : "Expires"} {formatDate(notification.expiryDate, locale)}</span>}</span>
                </span>
              </Link>
              );
            })}
